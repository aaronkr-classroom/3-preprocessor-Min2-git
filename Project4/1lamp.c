...
unsigned char ResetBit(
	...
	// 0 으로 설정하는 함수로 해당 비트가
	// 0으로 설정된 값을 반환합니다.

	//1 바이트 변수라서 비트 이동은 0~7까지 가능합니다.
	if (bit_num < 8)
		dest_data = dest_data & ~(0x01 << bit_num);
	// 변경된 값을 반환함
	return dest_data;

	//1100 1110 -> 4번쨍비트가 0으로 하고싶으면
	//0000 0001 = 0x01
	// 0001 0000= 0x01 << 4
	// 1111 1110 = ~(0x01 << 4)
	// Original: 
